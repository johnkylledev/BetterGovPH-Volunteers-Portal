...
// ---------- /projects (api/_handlers/projects.ts) ---------------------------
...
const handler_projects: H = async (req, res) => {
  ...
  try { const cached = await getCache<any[]>(cacheKey); if (cached && Array.isArray(cached)) { sendJson(res, 200, { projects: cached }); return; } }
  catch { /* noop */ }
  ...
  try { await setCache(cacheKey, projects, 300); } catch { /* noop */ }
